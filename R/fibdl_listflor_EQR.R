...
fibdl_listflor_EQR<- function(data,add_information_station){
  ...
  table_listflor_EQR <- data %>%
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(SESnor=dplyr::case_when(metrics=="DBO5"~ (SES-(-4.248347))/(0.9-(-4.25)),
                            metrics=="MES" ~ (SES-(  -4.158029 ))/(1.00-(  -4.16)),
                            metrics=="NKJ" ~ (SES-(-4.804299))/(1.00-(-4.80)),
                            metrics=="Ptot" ~ (SES-(-3.559219 ))/(1.173913-(-3.56))
    )
    ) %>%
    dplyr::ungroup() %>%
